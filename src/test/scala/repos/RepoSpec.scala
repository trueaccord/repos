...
class RepoSpec extends org.scalatest.fixture.FlatSpec with MustMatchers with OptionValues {
  ...
  "indexes" should "work" in {
    db =>
      ...
      await(db.run(FooRepo.lengthIndex.find(_.largerThan(1)))) must contain theSameElementsAs Seq(id2 -> "14", id4 -> "35", id3 -> "275")
      ...
      await(db.run(FooRepo.lengthIndex.countMatching(5))) must be (0)
      ...
      await(db.run(FooRepo.lengthIndex.max)).value must be(2)
